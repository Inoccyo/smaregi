...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If IsNothing(Session("TP")) Then
            ...
            If Not L_Func.GF_Initialize() Then
                Response.Redirect("./TP2990.html?CNWG051(Page_Load-1)")
                Exit Sub
            End If
            Session("TP") = L_Func
        Else
        ...
        If Trim$(Request.QueryString("SUBMITID")) = "btn_Set" Then
            ...
            txt_number_child.Text = Trim$(Request.QueryString("NUMBER_C"))
            LS_SetClick()
        End If
    End Sub
    ...
    Private Function LF_SetForm(Optional ByVal pNext As Boolean = False) As Object
        ...
        btn_0_Chi.Visible = True
        btn_1_Chi.Visible = True
        btn_2_Chi.Visible = True
        btn_3_Chi.Visible = True
        btn_4_Chi.Visible = True
        btn_5_Chi.Visible = True
        btn_6_Chi.Visible = True
        btn_7_Chi.Visible = True
        btn_8_Chi.Visible = True
        btn_9_Chi.Visible = True
        btn_cls_Chi.Visible = True

        txt_number.Text = "1"
        txt_number_child.Text = "0"
    End Function
    ...
    Private Sub LS_SetClick()
        ...
        'お通し処理の場合、人数未入力はだめ(大人)
        If Trim$(txt_number.Text) = "" Then
            Session(SessionMsgName) = "【入力エラー】人数(大人)を設定して下さい。"
            Exit Sub
        End If

        'お通し処理の場合、人数未入力はだめ(子供)
        If Trim$(txt_number_child.Text) = "" Then
            Session(SessionMsgName) = "【入力エラー】人数(子供)を設定して下さい。"
            Exit Sub
        End If

        'お通し処理の場合、人数がゼロはだめ(大人＋子供)
        If (CInt(Trim$(txt_number.Text)) + CInt(Trim$(txt_number_child.Text))) < 1 Then
    ...
    Private Function LF_SetSQLTPD010(ByVal pTERMID As String, ByVal pORDERNO As Long) As Object
        ...
        strSQL = strSQL & Format$(datNow, "yyyyMMddHHmmss") & ","                       '商品選択日時
        ...
        strSQL = strSQL & CLng(Trim$(txt_number.Text)) + CLng(Trim$(txt_number_child.Text)) & ", "                           'お通し人数
        ...
        'TPD014(INSERT文)
        strSQL = ""
        strSQL = strSQL & "INSERT INTO TPD014 VALUES("
        strSQL = strSQL & "'" & RSet(Request.QueryString("SEATID"), 5) & "', "          '座席番号
        strSQL = strSQL & pORDERNO & ", "                                               'オーダー連番
        strSQL = strSQL & "'" & pTERMID & "', "                                         '端末識別ID
        strSQL = strSQL & "'" & Trim$(Me.ViewState("OPEN_GOODSCD")) & "', "             '商品ID
        strSQL = strSQL & Format$(datNow, "yyyyMMddHHmmss") & ","                       '商品選択日時
        strSQL = strSQL & CLng(Trim$(txt_number.Text)) & ", "                           'お通し人数(大人)
        strSQL = strSQL & CLng(Trim$(txt_number_child.Text)) & ", "                     'お通し人数(子供)
        strSQL = strSQL & "'CNWG001', "                                                 '更新者
        strSQL = strSQL & "GETDATE())"                                                  '更新日
...
SYSTEM_ERROR:
        Response.Redirect("./TP2990.html?CNWG051(LF_SetSQLTPD010)")
        Exit Function
    End Function
...
SYSTEM_ERROR:
        Response.Redirect("./TP2990.html?CNWG051(LF_SetSQLTPM050)")
        Exit Function
    End Function
    ...
    Private Sub LF_SetSQLTPD050()
        ...
        strSQL = strSQL & "     D9 = '0', "
        strSQL = strSQL & "     D10 = '0' "
        strSQL = strSQL & "  UPDID  = 'CNWG051',"    '更新者
        strSQL = strSQL & "  UPDYMD = GETDATE()"     '更新日
        strSQL = strSQL & " WHERE SEATID = '" & RSet(Request.QueryString("SEATID"), 5) & "'"
        strSQL = strSQL & "   AND ( TERMKIND = 'TC0'"
        strSQL = strSQL & "   OR    TERMKIND = 'TCA' "
...
SYSTEM_ERROR:
        Response.Redirect("./TP2990.html?CNWG051(LF_SetSQLTPD050)")
        Exit Sub
    End Sub
    ...
    Private Function LF_GetDrinkMenuID() As String
        ...
        If Not L_Func.GF_GetData(strSQL, objData) Then
            Response.Redirect("./TP2990.html?CNWG051(LF_GetDrinkMenuID)")
            Exit Function
        End If
    ...
    Private Function LF_GetOrderNo() As Long
        ...
        If Not L_Func.GF_GetData(strSQL, objData) Then
            Response.Redirect("./TP2990.html?CNWG051(LF_GetOrderNo_1)")
            Exit Function
        End If
        ...
        If Not L_Func.GF_GetRows(objData, 0, objRow) Then
            Response.Redirect("./TP2990.html?CNWG051(LF_GetOrderNo_3)")
            Exit Function
        End If
    ...
    Public Sub OnInitBody()
        ...
        If Not L_Func.GF_GetData(strSQL, objData) Then
            Response.Redirect("./TP2990.html?CNWG051(LF_CheckOrderStatus-1)")
            Exit Sub
        End If
        ...
        If Not L_Func.GF_GetRows(objData, 0, objRow) Then
            Response.Redirect("./TP2990.html?CNWG051(LF_CheckOrderStatus-2)")
            Exit Sub
        End If
    ...
    Private Function LF_GetTermID(ByVal pINDEX As Long, ByRef pTERMID As String) As Boolean
        ...
        If Not L_Func.GF_GetData(strSQL, objDataSet) Then
            Response.Redirect("./TP2990.html?CNWG051(LF_GetTermID-1)")
            Exit Function
        End If
        ...
        If Not L_Func.GF_GetRows(objDataSet, pINDEX, objDataRow) Then
            Response.Redirect("./TP2990.html?CNWG051(LF_GetTermID-2)")
            Exit Function
        End If
    ...
    Private Function LF_GetMenuName() As String
        ...
        If Not L_Func.GF_GetData(strSQL, objData) Then
            Response.Redirect("./TP2990.html?CNWG051(LF_GetMenuName-1)")
            Exit Function
        End If
        ...
        If Not L_Func.GF_GetRows(objData, 0, objRow) Then
            Response.Redirect("./TP2990.html?CNWG051(LF_GetMenuName-2)")
            Exit Function
        End If
    ...
    Private Sub LS_ButtonProtect()
        ...
        btn_0_Chi.Attributes("ONCLICK") = "return false"
        btn_1_Chi.Attributes("ONCLICK") = "return false"
        btn_2_Chi.Attributes("ONCLICK") = "return false"
        btn_3_Chi.Attributes("ONCLICK") = "return false"
        btn_4_Chi.Attributes("ONCLICK") = "return false"
        btn_5_Chi.Attributes("ONCLICK") = "return false"
        btn_6_Chi.Attributes("ONCLICK") = "return false"
        btn_7_Chi.Attributes("ONCLICK") = "return false"
        btn_8_Chi.Attributes("ONCLICK") = "return false"
        btn_9_Chi.Attributes("ONCLICK") = "return false"
        btn_cls_Chi.Attributes("ONCLICK") = "return false"
    End Sub
    ...
    Private Sub LS_ButtonNotProtect()
        ...
        btn_0_Chi.Attributes("ONCLICK") = "return true"
        btn_1_Chi.Attributes("ONCLICK") = "return true"
        btn_2_Chi.Attributes("ONCLICK") = "return true"
        btn_3_Chi.Attributes("ONCLICK") = "return true"
        btn_4_Chi.Attributes("ONCLICK") = "return true"
        btn_5_Chi.Attributes("ONCLICK") = "return true"
        btn_6_Chi.Attributes("ONCLICK") = "return true"
        btn_7_Chi.Attributes("ONCLICK") = "return true"
        btn_8_Chi.Attributes("ONCLICK") = "return true"
        btn_9_Chi.Attributes("ONCLICK") = "return true"
        btn_cls_Chi.Attributes("ONCLICK") = "return true"
    End Sub
    ...
    Private Function LF_SetSQLTPD010_2(ByVal pTERMID As String, ByVal pORDERNO As Long, ByVal pCNT As String) As Object
        ...
        If Len(strSUBGOODSCD) <> 0 And Len(Trim$(txt_number.Text)) <> 0 Then
            If strSUBGOODSCD <> "0000" And Trim$(txt_number.Text) <> 0 Then
                ...
                strSQL = strSQL & Format$(datNow, "yyyyMMddHHmmss") & ","                   '商品選択日時
                strSQL = strSQL & Format$(datNow, "yyyyMMddHHmmss") & ","                   '注文受付日時
                strSQL = strSQL & "0, "                                                     'ステーション配信日時
                strSQL = strSQL & CLng(Trim$(txt_number.Text)) + CLng(Trim$(txt_number_child.Text)) & ", "                             '数量
                strSQL = strSQL & "'00', "                                                  '指示ステータス位置１
                strSQL = strSQL & "'000', "                                                 'SCPｺｰﾄﾞ１
                strSQL = strSQL & "'0000', "                                                'SCPﾒﾆｭｰ１
                strSQL = strSQL & "0, "                                                     'SCP数量１
...
SYSTEM_ERROR:
        Response.Redirect("./TP2990.html?CNWG051(LF_SetSQLTPD010_2)")
        Exit Function
    End Function
