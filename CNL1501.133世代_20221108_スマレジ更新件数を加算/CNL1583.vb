...
#Region " Windows フォーム デザイナで生成されたコード "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.txt_Title.Location = New System.Drawing.Point(24, 40)
        ...
        Me.btn_Cancel.Location = New System.Drawing.Point(360, 72)
        ...
        Me.btn_OK.Location = New System.Drawing.Point(256, 72)
        ...
        Me.Label1.Text = "■ 新しく作成するコースメニュー名を入力してください"
...
#End Region
    ...
    Private Sub CNL1583_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
SYSTEM_ERROR:
    ...
    Private Function LF_InsertNewMenu() As Boolean
        ...
        'If Not GF_GetGoodsCD(strGoodsCD1, strGoodsCD2, 8500) Then
        If Not GF_GetGoodsCD(strGoodsCD1, strGoodsCD2, 7500) Then
        ' --- ↑(2008.11.25) 皿勘定有時の飲み放題対応のため修正↑ ---
            Exit Function
        End If

        ' コースメニュー商品コードを商品マスタ(TPM010)に登録
        If Not GF_InTPM010(strGoodsCD1, Trim$(txt_Title.Text)) Then
            Exit Function
        End If

        strSQL = ""
        strSQL = strSQL & "INSERT INTO TPM180 VALUES("
        strSQL = strSQL & "'" & strGoodsCD1 & "',"                  ' 商品コード
        strSQL = strSQL & "'" & LF_GetDefMenuID() & "',"            ' タッチパネル表示メニュー
        strSQL = strSQL & "'000',"                                  ' 飲み放題メニュー
        ...
        '--- (2006.06.16) STN100バージョンアップ対応 HTL用商品不要 -----------------------
        '' ハンディ用コースメニュー商品コード採番
        'If Not GF_GetGoodsCD(strGoodsCD1, strGoodsCD2, 7500) Then
        '    Exit Function
        'End If

        '' ハンディ用コースメニュー商品コードを商品マスタ(TPM010)に登録
        'If Not GF_InTPM010(strGoodsCD1, Trim$(txt_Title.Text)) Then
        '    Exit Function
        'End If
        '---------------------------------------------------------------------------------
    ...
    Private Function LF_GetDefMenuID() As String
        ...
        strSQL = ""
        strSQL = strSQL & "SELECT DEFMENUID FROM TPM040"
        ...
        If objData.Tables("com").Rows.Count > 0 Then
            If Not GF_GetRows(objData, 0, objRow) Then
                Exit Function
            End If
            LF_GetDefMenuID = objRow("DEFMENUID")
        End If
